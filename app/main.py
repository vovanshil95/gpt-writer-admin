...
class InteractionShema(BaseModel):
    prompts: list[str]
    gpt_response: str
    datetime: datetime.datetime

class InteractionsResponse(BaseResponse):
    data: list[InteractionShema]

class GptAnswerShema(BaseModel):
    gpt_response: str
...
def get_questions() -> ResponseQuestions:
    with sqlalchemy_session.begin() as session:
        questions = session.query(Question).all()
        questions = list(map(lambda q: {'id': q.id, 'text_data': q.text_data}, questions))
...
@app.get('/api/prompt')
def get_prompt() -> ResponsePrompts:
    with sqlalchemy_session.begin() as session:
...
@app.put('/api/prompt')
def put_prompt(prompts: list[str]) -> ResponsePrompts:
    with sqlalchemy_session.begin() as session:
